...
template<typename T> class RingBuffer {//环形缓冲区域类即为循环队列
 public:
  explicit RingBuffer(size_t size)//explicit构造函数是用来防止隐式转换的
    : size_(size)
    , begin_(0)
    , end_(0)
    , wrap_(false) {
  ...
  RingBuffer(const RingBuffer<T> & rb) {//拷贝构造函数
    //this(rb.size_);
    size_ = rb.size_;
    begin_ = rb.begin_;
    end_ = rb.end_;
 ...
 private:
  ...
  bool wrap_;
  std::mutex mtx_;
  inline size_t GetFreeSize() {//内联函数提高函数的执行效率
    size_t occ = 0;
    if (end_ == begin_) {
      occ = wrap_ ? size_ : 0;
    }
