...
TrackObject::TrackObject(UsbSerial& usb_serial, const std::vector<cv::Point3d>& vertices, size_t packet_size, size_t serial_data_size) :
  Consumer<LightSensorDataPacket>(data_packet_),//initializing the direct base class "Consumer"
...
bool TrackObject::Product() {
  ...
  if (begin_pos == -1) {
    ...
    memcpy(data_, buffer + begin_pos, serial_data_size_ - begin_pos);//从buffer中读取有效数据到data_
  }
  int ret = usb_serial_.Read(data_ + serial_data_size_ - begin_pos, begin_pos); // read the rest data.读取剩余数据到data_
  if (!ret) return ret;

  if (CheckDataTail()) {
    LightSensorDataPacket lsdp;
    memcpy(&lsdp, data_ + 2, sizeof(LightSensorDataPacket));//read the useful data to LightSensorDataPacket
...
void TrackObject::Join() {//not used
  for (auto & it : thread_) {
    if (it.joinable())
      it.join();
  }
