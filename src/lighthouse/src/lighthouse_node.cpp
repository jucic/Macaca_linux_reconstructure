...
bool ReadPoints(std::vector<cv::Point3d>& pts, std::string file) {
  int nums = 0;
  std::ifstream fin(file, std::fstream::in);//open the file in a read only mode
  if (!fin.is_open()) return false;
  fin >> nums;//number of points, here is 36
  double x = 0, y = 0, z = 0;
...
int main(int argc, char* argv[]) {
  ...
  nh.getParam("lighthouse_frame", lighthouse_frame);//lighthouse_link
  nh.getParam("marker_frame", marker_frame);//eyetracker_link
  nh.getParam("world_frame", world_frame);

  cout << usb_device_name << " " << object_points_file << endl;
  UsbSerialLinux usb_serial(usb_device_name);//open the usb while initializing
